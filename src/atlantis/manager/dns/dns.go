...
func (a *Alias) Id() string {
	return strings.Replace(strings.Replace(fmt.Sprintf("%s.%s.%s", a.Alias, a.Original, a.Failover), "-", "", -1), ".", " ", -1)
}
...
func (a *ARecord) Id() string {
	return strings.Replace(strings.Replace(fmt.Sprintf("%s.%s.%s", a.Name, a.IP, a.Failover), "-", "", -1), ".", " ", -1)
}
