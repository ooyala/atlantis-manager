...
func (s *DatamodelSuite) TestManagerPath(c *C) {
	...
	CreateManagerPath()
	opt := Manager("dev", "1.1.1.1", "2.1.1.1")
...
func (s *DatamodelSuite) TestManagerListers(c *C) {
	...
	devOpt := Manager("dev", "1.1.1.1", "2.1.1.1")
	c.Assert(devOpt.Save(), IsNil)
	devOtherOpt := Manager("dev", "1.1.1.2", "2.1.1.2")
	c.Assert(devOtherOpt.Save(), IsNil)
	omgOpt := Manager("omg", "1.1.1.3", "2.1.1.3")
	c.Assert(omgOpt.Save(), IsNil)
	bbqOpt := Manager("bbq", "1.1.1.4", "2.1.1.4")
