...
func (e *DeployExecutor) Execute(t *Task) error {
	...
	if e.arg.App != manifest.Name {
		/* NOTE(edanaher): If we kick off two jobs simultaneously, they will assume they have the same job id, so
						 * one of them will get the manifest from the other one's Jenkins job.  Unfortunately, Jenkins doesn't give
						 * back any sort of useful information when you create the job, so we can't just use an ID easily.
						 * Moreover, the API may not in any way acknowledge the job for several seconds, meaning that we can't even
						 * scan the jobs created during a brief time interval for the job in question.  Rather, we have to poll
						 * Jenkins until we find the job we're looking for.
						 * This is sufficiently terrible that I'm just erroring out and blaming Jenkins rather than adding a giant
						 * pile of code to handle that case.
				     * We could retry the job ourself, but after a day trying to beat Jenkins into submission, I have no
				     * interest in applying further hacks.
		         * Also, go fmt appears to horribly break the formatting of this comment.  *sigh*
		*/
