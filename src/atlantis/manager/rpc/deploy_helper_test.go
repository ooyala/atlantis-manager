...
func (s *DeployHelperSuite) TestResolveDepValues(c *C) {
	...
	c.Assert(err, IsNil)
	_, err = datamodel.CreateOrUpdateApp(false, "app", "ssh://github.com/ooyala/app", "/", "jigish@ooyala.com")
	c.Assert(err, IsNil)
	zkApp, err := datamodel.CreateOrUpdateApp(false, "hello-go", "ssh://github.com/ooyala/hello-go", "/", "jigish@ooyala.com")
	c.Assert(err, IsNil)
